...
- Invoke events on the server and wait for correlated responses (preview)
...
If invocation fails, `InvokeEventFailedException` is thrown and includes `InvocationId` and optional `ErrorDetail` from the service.
